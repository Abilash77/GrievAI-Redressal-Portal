...
const LodgeComplaint: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Predict Priority
      const priority = await predictPriority(formData.title, formData.description, formData.department);

      // 2. Process Files (Convert to Base64 mock)
      let voiceUrl = '';
      if (voiceBlob) {
      ...
      setSubmitResult({ id: complaint.id, priority: complaint.priority });
    } catch (error) {
      console.error(error);
